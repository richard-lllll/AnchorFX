...
public class DockCommandsBox extends HBox {
  ...
  private void createCloseButton() {
    ...
    node.floatingProperty()
        .addListener((observer, oldValue, newValue) -> changeStateForFloatingState());
  ...
  void notifyCloseAction() {
    if (isMenuOpen) {
      isMenuOpen = false;
      RotateTransition rotate = new RotateTransition(Duration.seconds(0.2),
          menuButton.getGraphic());
      rotate.setToAngle(0);
      rotate.play();
      hideAction.run();
    }
