...
public final class DockStation extends SingleDockContainer {
  ...
  public boolean isInnerPosition(double x, double y) {
    ...
    Rectangle bounds = new Rectangle(window.getX(), window.getY(), window.getWidth(),
        window.getHeight());
  ...
  private void manageDragOnSameNode() {
    DockTabberContainer tabContainer = (DockTabberContainer) dockZones.getCurrentNodeTarget()
        .getParentContainer();
    tabContainer
        .manageDragOnSameNode(dockZones.getCurrentNodeTarget(), dockZones.getCurrentPosition());
  }
  ...
  public void finalizeDrag() {
    ...
    if (selector == null) {
      dockZones.getNodeSource()
          .makeNodeActiveOnFloatableStage(getScene().getWindow(), getScene().getX(),
              getScene().getY());
    } else {
  ...
  private void manageDockDestination() {
    if (dockZones.getCurrentNodeTarget() == dockZones.getNodeSource()) {
      ...
      dockZones.getNodeSource().undock();
      dockZones.getNodeSource()
          .dock(dockZones.getCurrentNodeTarget(), dockZones.getCurrentPosition());
    }
  }
